-- Update: 2026-03-14
-- Purpose: Align winner count with round rule (Round 3 => top 3 winners),
-- while keeping re-settlement idempotent and phase-safe timeout behavior.
...
begin
  ...
  if v_participant_count > 0 then
    -- Round rule winner cap, limited only by participating teams.
    v_winner_count := least(v_winner_count, v_participant_count);
  else
    raise exception 'No evaluated teams found for round %', p_round_no;
  end if;
