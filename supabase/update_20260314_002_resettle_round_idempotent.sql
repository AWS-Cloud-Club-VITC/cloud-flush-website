-- Update: 2026-03-14
-- Purpose: Make settlement idempotent for re-settling already settled rounds,
-- and avoid timeout-hold call outside decision phase.
...
begin
  ...
  if v_ctrl.phase not in ('decision', 'evaluation', 'settled') then
    raise exception 'Round % must be in decision/evaluation/settled phase before settlement', p_round_no;
  end if;

  -- Only apply timeout hold when the round is actually in decision phase.
  if v_ctrl.phase = 'decision' then
    perform public.apply_hold_timeouts(p_round_no);
  end if;

  -- If this round was already settled before, rollback prior settlement deltas first
  -- so re-settling does not double-count points.
  with prior as (
    select bt.team_id, coalesce(sum(bt.delta_points), 0) as delta_sum
    from public.betting_transactions bt
    where bt.round_no = p_round_no
      and bt.reason like 'Round settlement%'
    group by bt.team_id
  )
  update public.teams t
  set points = t.points - prior.delta_sum
  from prior
  where t.id = prior.team_id;

  delete from public.betting_transactions bt
  where bt.round_no = p_round_no
    and bt.reason like 'Round settlement%';
